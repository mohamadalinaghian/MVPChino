...
def pos_dashboard(request):
    ...
    open_orders = Order.objects.filter(status=Order.OrderStatus.OPEN).order_by(
        "-created_at"
    )
    ...
    if current_order_id:
        current_order = Order.objects.filter(
            uuid=current_order_id, status=Order.OrderStatus.OPEN
        ).first()
...
def start_new_order(request, table_id):
    ...
    order = Order.objects.create(table=table, status=Order.OrderStatus.OPEN)
...
def switch_order(request, order_uuid):
    order = get_object_or_404(Order, uuid=order_uuid, status=Order.OrderStatus.OPEN)
    request.session["current_order_id"] = str(order.uuid)
...
def add_to_cart(request, item_id):
    ...
    if not current_order_id:
        return HttpResponse(
            "<div class='cart-empty'>لطفا ابتدا یک میز یا سفارش باز انتخاب کنید.</div>"
        )

    order = get_object_or_404(
        Order, uuid=current_order_id, status=Order.OrderStatus.OPEN
    )
